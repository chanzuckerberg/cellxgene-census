...
# TODO: DRY with cellxgene_census_builder/../mp.py
...
def _hard_process_cap(max_worker_processes: int, n_proc: int) -> int:
    """
    ...
    """
    return min(int(max_worker_processes), n_proc)
...
def _default_worker_process_count(max_worker_processes: int) -> int:
    """Return the default worker process count, subject to configured limit."""
    return _hard_process_cap(max_worker_processes, _cpu_count() + 2)


def n_workers_from_memory_budget(
    ...
    memory_budget = psutil.virtual_memory().total
    n_workers: int = int(memory_budget // per_worker_budget)
    return min(n_workers, _default_worker_process_count(max_worker_processes))


def create_process_pool_executor(
    max_worker_processes: int,
    max_workers: Optional[int] = None,
    process_init: Optional[Callable[[], None]] = None,
    init_args: Tuple[Any, ...] = (),
) -> ProcessPoolExecutor:
    assert _mp_config_checks()
    if max_workers is None:
        max_workers = _default_worker_process_count(max_worker_processes)
    logging.debug(f"create_process_pool_executor [max_workers={max_workers}]")
    return ProcessPoolExecutor(max_workers=max_workers, initializer=process_init, initargs=init_args)
...
class _Scheduler(threading.Thread):
    def __init__(self, executor: "ResourcePoolProcessExecutor", max_resources: int):
        ...
        assert isinstance(executor.process_pool, Pool)
    ...
    def _get_work(self) -> _MightBeWork:
        ...
        * second element is an optional work item. Will be None in case of shutdown
        ...
        def _get_next_work() -> Optional[int]:
    ...
    @classmethod
    def _work_item_done(
        cls, scheduler: "_Scheduler", wi: _WorkItem[_T], is_error_callback: bool, result: Union[_T, BaseException]
    ) -> None:
...
class ResourcePoolProcessExecutor(contextlib.AbstractContextManager["ResourcePoolProcessExecutor"]):
    ...
    def submit(self, resources: int, fn: Callable[_P, _T], *args: _P.args, **kwargs: _P.kwargs) -> Future[_T]:
        f = Future[_T]()
        work_item = _WorkItem[_T](resources=resources, future=f, fn=fn, args=args, kwargs=kwargs)
        self.scheduler.submit(work_item)
        return f
...
def create_resource_pool_executor(
    ...
    process_init: Optional[Callable[[], None]] = None,
    process_init_args: Tuple[Any, ...] = (),
) -> ResourcePoolProcessExecutor:
    assert _mp_config_checks()

    if max_resources is None:
        max_resources = memory_budget
        assert max_resources is not None, "must specify either max_resources or memory_budget"
    if max_workers is None:
        max_workers = _default_worker_process_count(max_worker_processes)
