...
def _open_soma(description: CensusVersionDescription) -> soma.Collection:
    ...
    return soma.Collection(uri=locator["uri"], context=SOMATileDBContext(tiledb_ctx=tiledb.Ctx(tiledb_config)))
...
def open_soma(*, census_version: Optional[str] = "latest", uri: Optional[str] = None) -> soma.Collection:
    ...
    if uri is not None:
        return soma.Collection(uri=uri, context=SOMATileDBContext(tiledb_ctx=tiledb.Ctx(DEFAULT_TILEDB_CONFIGURATION)))
