...
The `cellxgene.census` R package  contains a convenient `open_soma()` API to open any version of the Census (the latest by default).
...
With the command above you created `census`, which is a `SOMACollection`, an R6 class supplying providing a key-value associative map. Its `get()` method can access the two top-level collection members, `census_info` and `census_data`, each themselves instances of `SOMACollection`.
...
```{r}
as.data.frame(census$get("census_info")$get("summary")$read())
```
...
```{r}
census_counts <- as.data.frame(census$get("census_info")$get("summary_cell_counts")$read())
census_counts
```
...
```{r}
as.data.frame(census$get("census_data")$get("homo_sapiens")$obs$read(column_names=c("cell_type", "is_primary_data")))
```

The number of rows is the total number of cells for humans. Now, if you wish to get the cell counts per cell type we can work with this data framet.
...
```{r}
human_liver_cell_types <- as.data.frame(
  census$get("census_data")$get("homo_sapiens")$obs$read(
    column_names="cell_type",
    value_filter="is_primary_data == TRUE && tissue_general == 'liver")
)$cell_type
t(table(human_liver_cell_types))
```
...
```{r}
t_cells_diseased <- as.data.frame(
  census$get("census_data")$get("homo_sapiens")$obs$read(
