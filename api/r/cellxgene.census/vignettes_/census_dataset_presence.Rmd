...
The presence matrix has a method `$take()` that let's you slice it by the `soma_joinid` from dataset and var joinid. And the full presence matrix or slices of it can be then exported to a regular matrix with the method `$get_one_based_matrix()`

Let's find out if the the gene `"ENSG00000286096"` was measured in the dataset with id `"97a17473-e2b1-4f31-a544-44a60773e2dd"`.

```{r}
# Get somo_joinid for datasets and genes of interest
