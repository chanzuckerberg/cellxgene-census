...
liver_seurat <- cellxgene.census::get_seurat(
  ...
  obs_value_filter = paste("dataset_id == '", liver_dataset_id, "'", sep="")
)
```
...
```{r}
liver_seurat[1:5, 1:5]$RNA@data
```

```{r}
gene_lengths <- liver_seurat$RNA@meta.features$feature_length
liver_seurat <- Seurat::SetAssayData(
  liver_seurat, 
  new.data = sweep(liver_seurat$RNA@data, 1, gene_lengths, "/")
)
```
...
```{r}
cells_per_gene <- Matrix::rowSums(liver_seurat$RNA@counts > 0)
...
liver_seurat <- Seurat::RunPCA(
  liver_seurat,
  features = Seurat::VariableFeatures(object = liver_seurat)
)
liver_seurat <- Seurat::FindNeighbors(liver_seurat, dims = 1:10)
liver_seurat <- Seurat::RunUMAP(liver_seurat, dims = 1:10)
Seurat::DimPlot(liver_seurat, reduction = "umap", group.by = "cell_type")
```
